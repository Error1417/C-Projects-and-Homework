...
const int COLS = 30; // constant for the 2d array
...
void extFunc(float);
//welcome text in arrays
void welcome(char [],char[],vector<char>, int);
...
int main(int argc, char** argv) 
{
    ...
    //size constant for the arrays
    const int SIZE = 30;
    const int SIZETW = 3;
    //These are examples of arrays and vectors
    char lines[SIZE] = {'-','-','-','-','-','-','-','-','-','-','-','-','-','-',
                         '-','-','-','-','-','-','-','-','-','-'};
    char welcom[SIZE] = {'|','-','W','E','L','C','O','M','E',' ','T','O',' ',
                         'B','L','A','C','K','J','A','C','K','-','|'};
    vector<char> line= {'-','-','-','-','-','-','-','-','-','-','-','-','-','-',
                         '-','-','-','-','-','-','-','-','-','-'};
    
    //2d array 
    char end[SIZETW][COLS] = {{'-','-','-','-','-','-','-','-','-','-','-','-','-','-',
                         '-','-','-','-','-','-','-','-','-','-'},
                         {'|','-','-','-','-','-','E','N','D',' ','O','F',' ',
                         'G','A','M','E','-','-','-','-','-','-','|'},
                         {'-','-','-','-','-','-','-','-','-','-','-','-','-','-',
    ...
    welcome(lines,welcom,line,SIZE);
    cout << endl;
    ...
    do 
    {
        //a function that checks the players balance if its below 0$
        extFunc(balance);
    ...
    // end of game array output
    // uses a 2d array
    for(int i = 0; i < SIZETW;i++){
...
int cardVal(string card1, string card2, string card3, string card4, string card5, string card6,string card7, 
    ...
    //this function returns the ace status and the card value to add it to the dealer or the player
    return aceStat;
    return cardVal;
}
...
int cardVal(string card1, string card2, string card3, string card4, string card5, string card6,string card7, 
    ...
    //this function returns one card value when the player or dealer selects to hit
    return cardVal;
}
...
void welcome(char lines[],char welc[],vector<char> line , int n){
    
    //Example of static casting
    n = static_cast<short>(n);
    
    //loops used to output the arrays and vector
